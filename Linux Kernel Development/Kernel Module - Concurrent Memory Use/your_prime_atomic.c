...
static void non_primes(int *counter){
	...
	while(1){
		...
		while (current_pos < upper_bound-1){
			if(atomic_read(&integers[current_pos]) != 0)
				break;
			else
				current_pos++;
		}	
		...
		if (position >= upper_bound-1) 
			...
			num = atomic_read(&integers[position]);
			for(i = position + num; i < upper_bound-1; i = i + num){
				atomic_set(&integers[i], 0);
				(*counter)++;
			}
		}
	}
...
static void exit_function(void){
	...
	for(i = 0; i < upper_bound-1; i++){
		if(atomic_read(&integers[i]) != 0){
			printk(KERN_CONT "%d ", atomic_read(&integers[i]));
			primes++;
			if (primes % 8 == 0) {
				printk(KERN_CONT "\n");
			}
